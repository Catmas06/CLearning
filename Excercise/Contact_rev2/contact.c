...
void initContact(Contact *src)
{
    ...
    {
        perror("fopen");
        printf("源文件不存在，新建空通讯录\n");
        return;
    }
    //读文件
    PeopleInFo buf;
    int count = 0;
    while (fread(&buf, sizeof(PeopleInFo), 1, fp))
    {
        src->data[count] = buf;
        count++;
        src->size++;
...
void saveToFile(Contact *src)
{
    ...
    while (i < src->size)
    {
        fwrite(src->data + i, sizeof(PeopleInFo), 1, fp);
        i++;
    }
    //关闭文件
    fclose(fp);
